...
class TestUpscaleByMaxSide(unittest.TestCase):
    def setUp(self):
        # Need to mock comfy.utils before importing Upscale_By_Max_Side
        import comfy.utils
        self.original_upscale = comfy.utils.common_upscale
        # Replace with identity-like mock for testing dimension logic
    ...
    def test_upscale_landscape(self):
        ...
        result = self.node.upscale(Image=img, Max_Side=1024, Divisibility=8, Method="bilinear")
    ...
    def test_min_dimension_not_zero(self):
        ...
        result = self.node.upscale(Image=img, Max_Side=64, Divisibility=8, Method="bilinear")
        w, h = result[1], result[2]
    ...
    def test_aspect_ratio_preserved(self):
        ...
        result = self.node.upscale(Image=img, Max_Side=1024, Divisibility=1, Method="bilinear")
        _, w, h = result[0].shape[1], result[1], result[2]
        input_ratio = 800 / 400
        output_ratio = w / h
    ...
    def test_batch_dimension_preserved(self):
        ...
        result = self.node.upscale(Image=img, Max_Side=512, Divisibility=8, Method="bilinear")
